 ...
 *   public/data/sensor-events.json    - Full event log with offline/online history
 *
...
// YoLink water depth sensors typically report every 4 hours
const OFFLINE_THRESHOLD_MS = 5 * 60 * 60 * 1000; // 5 hours
...
const sensorStatus = {};    // { deviceId: 'online' | 'offline' }
...
function logEvent(type, deviceId, deviceName, details = {}) {
  ...
  const event = {
    ...
    type,            // 'online', 'offline', 'reading', 'startup', 'system', 'hub_offline', 'hub_online'
    deviceId,
    deviceName,
    details
  };
  ...
  if (deviceId) {
    ...
    if (type === 'online') {
      ...
      if (sensor.lastOfflineAt) {
        const offlineDuration = new Date(event.timestamp) - new Date(sensor.lastOfflineAt);
        event.details.offlineDurationMs = offlineDuration;
        event.details.offlineDurationHuman = formatDuration(offlineDuration);
      }
  ...
  // Console output
  const icon = type === 'online' ? '🟢' : type === 'offline' ? '🔴' : type === 'reading' ? '📊' : '⚙️';
...
function checkForOfflineSensors() {
  ...
  for (const [deviceId, info] of Object.entries(sensorLastSeen)) {
    ...
    if (timeSinceLastMessage > OFFLINE_THRESHOLD_MS && sensorStatus[deviceId] !== 'offline') {
...
function processMessage(topic, payload) {
  try {
    ...
    const wasOffline = sensorStatus[deviceId] === 'offline';
    ...
    if (knownDevice || deviceEvent.includes('WaterDepthSensor') ||
      ...
      logEvent('reading', deviceId, deviceName, {
        depth: reading.level,
...
async function connect() {
  ...
  tokenRefreshInterval = setInterval(async () => {
    try {
      console.log('Refreshing MQTT access token...');
