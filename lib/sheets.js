...
class GoogleSheetsClient {
  ...
  /**
   * Get distribution leak data from the "Service Meters" tab
   * Compares distribution meter vs house meter usage quarterly
   * Row 1: date headers, Row 170: total gallons, Row 171: house gpd,
   * Row 172: distribution gpd, Row 173: gap gpd
   */
  async getDistributionLeakData() {
    console.log('Fetching distribution leak data...');

    try {
      const serviceMetersTab = 'Service Meters';

      const [dateHeaders, totalUsageRow, houseGpdRow, distGpdRow, gapRow] = await Promise.all([
        this.readSheet(this.productionSheetId, `'${serviceMetersTab}'!F1:Q1`),
        this.readSheet(this.productionSheetId, `'${serviceMetersTab}'!F170:Q170`),
        this.readSheet(this.productionSheetId, `'${serviceMetersTab}'!F171:Q171`),
        this.readSheet(this.productionSheetId, `'${serviceMetersTab}'!F172:Q172`),
        this.readSheet(this.productionSheetId, `'${serviceMetersTab}'!F173:Q173`),
      ]);
      ...
      const dates = (dateHeaders[0] || []);
      const totals = (totalUsageRow[0] || []);
      const houseGpd = (houseGpdRow[0] || []);
      const distGpd = (distGpdRow[0] || []);
      const gaps = (gapRow[0] || []);
      ...
      for (let i = 0; i < dates.length; i++) {
        ...
        const houseMeterTotal = parseNum(totals[i]);
        const houseMeterGpd = parseNum(houseGpd[i]);
        const distributionGpd = parseNum(distGpd[i]);
        const gapGpd = parseNum(gaps[i]);

        if (houseMeterGpd === null && distributionGpd === null) continue;

        // Use gap from sheet if available, otherwise compute from distribution - house
        let effectiveGap = gapGpd;
        if (effectiveGap === null && distributionGpd !== null && houseMeterGpd !== null) {
          effectiveGap = distributionGpd - houseMeterGpd;
        }

        let lossPercent = null;
        if (distributionGpd !== null && distributionGpd > 0 && effectiveGap !== null) {
          lossPercent = (effectiveGap / distributionGpd) * 100;
        }
        ...
        quarters.push({
          ...
          distributionGpd,
          gapGpd: effectiveGap,
          lossPercent: lossPercent !== null ? Math.round(lossPercent * 10) / 10 : null
        });
      }

      console.log(`✓ Found ${quarters.length} quarterly distribution leak records`);
      return quarters;

    } catch (error) {
      console.error('Error fetching distribution leak data:', error.message);
      throw error;
    }
  }
